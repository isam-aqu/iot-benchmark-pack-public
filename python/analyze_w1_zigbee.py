...
DEFAULT_RUN_ID = "W1_zigbee_near_quiet"
DEFAULT_NODE_ID = "light.benchmark_device"
...
NODE_ID = sys.argv[3] if len(sys.argv) > 3 else os.getenv("NODE_ID", DEFAULT_NODE_ID)

if not EXP_RUN_ID:
    raise SystemExit("EXP_RUN_ID is required, e.g. EXP_RUN_ID=R202")
...
df = df[(df["protocol"] == "zigbee") & (df["type"] == "event")].copy()
df = df[df["node"] == NODE_ID].copy()

df["pi_time_s"] = pd.to_numeric(df["pi_rx_time_ns"], errors="coerce") / 1e9
df["ha_time_s"] = df["ha_time"].apply(lambda x: datetime.fromisoformat(x).timestamp() if pd.notna(x) else None)

df["raw_delay_ms"] = (df["pi_time_s"] - df["ha_time_s"]) * 1000.0
...
def print_stats(label: str, data: pd.DataFrame):
    ...
    print(f"[{label}] Zigbee normalized delay statistics (ms):")
    print(f"Count:   {len(s)}")
...
if "event_id" in df.columns:
    print("Unique event_id count:", df["event_id"].nunique(dropna=True))

print("State transitions:")
print(df[["old_state", "new_state"]].value_counts())

df_full["pipeline_version"] = PIPELINE_VERSION
df_core["pipeline_version"] = PIPELINE_VERSION

df_full.to_csv(derived_dir / f"{RUN_ID}_zigbee_events_processed_full.csv", index=False)
df_core.to_csv(derived_dir / f"{RUN_ID}_zigbee_events_processed.csv", index=False)
