...
INA226_WE::INA226_WE(uint8_t address, TwoWire* wire)
    : address_(address),
      wire_(wire),
...
void INA226_WE::setResistorRange(float shuntResistanceOhm, float maxCurrentA) {
...
bool INA226_WE::writeRegister16(uint8_t reg, uint16_t value) {
  wire_->beginTransmission(address_);
  wire_->write(reg);
...
bool INA226_WE::readRegister16(uint8_t reg, uint16_t& value) {
  wire_->beginTransmission(address_);
  wire_->write(reg);
...
int16_t INA226_WE::readSignedRegister16(uint8_t reg) {
  uint16_t value = 0;
...
uint16_t INA226_WE::readUnsignedRegister16(uint8_t reg) {
  uint16_t value = 0;
