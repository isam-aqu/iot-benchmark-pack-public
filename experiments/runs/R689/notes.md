## R689 - W3 Zigbee AP-Channel Extension

Run ID: `W3_zigbee_auto_heavy_ch9`

Protocol: `zigbee`

Purpose: AP channel 9 / 20 MHz overlap heavy-load run paired with baseline `R688`.
...
- Max latency: `13.5 ms`
- Loss rate: `0.0%`
- P95 change vs `R688`: `1.0 ms` (`9.3%`)

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R689/raw --run-id W3_zigbee_auto_heavy_ch9 --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R689/raw --run-id R689 --run-sheet docs/run_sheet.csv
```

Interpretation:

This is a valid AP ch9/20 MHz heavy-load replicate paired with `R688`. The reliability caveat is reflected in the data-quality bullets above.
