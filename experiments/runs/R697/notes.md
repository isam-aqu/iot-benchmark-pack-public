## R697 - W3 Zigbee AP-Channel Extension

Run ID: `W3_zigbee_auto_heavy_ch9`

Protocol: `zigbee`

Purpose: AP channel 9 / 20 MHz overlap heavy-load run paired with baseline `R696`.
...
- Max latency: `12.7 ms`
- Loss rate: `0.0%`
- P95 change vs `R696`: `-0.3 ms` (`-2.6%`)

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R697/raw --run-id W3_zigbee_auto_heavy_ch9 --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R697/raw --run-id R697 --run-sheet docs/run_sheet.csv
```

Interpretation:

This is a valid AP ch9/20 MHz heavy-load replicate paired with `R696`. The run has complete command/event coverage with no reliability caveats.
