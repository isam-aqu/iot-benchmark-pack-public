## R662 - W3 BLE AP-Channel Extension
...
```bash
EXP_RUN_ID=R662 ./run_all.sh --run-id W3_ble_auto_baseline --enable-ble --disable-ack --duration-sec 360 --serial-port /dev/serial-port --serial-baud 115200 --iperf-load 0 --reset-esp32
```

Completion:

```text
EXPERIMENT_COMPLETE,exp=W3_ble_auto,boot_id=1315726357,reason=target_event_count,duration_ms=302851,events=100
```

Data quality:

- BLE captured events: 98/100
- Missing sequences: `27, 56`
...
- MAD latency: `90.5 ms`
- P95 latency: `171.8 ms`
- P99 latency: `356.5 ms`
- Max latency: `368.7 ms`
- Loss rate: `2.0%`

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R662/raw --run-id W3_ble_auto_baseline --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R662/raw --run-id R662 --run-sheet docs/run_sheet.csv
```
