## R677 - W3 BLE AP-Channel Extension
...
```bash
EXP_RUN_ID=R677 ./run_all.sh --run-id W3_ble_auto_heavy --enable-ble --disable-ack --duration-sec 360 --serial-port /dev/serial-port --serial-baud 115200 --iperf-load heavy --reset-esp32
```

Completion:

```text
EXPERIMENT_COMPLETE,exp=W3_ble_auto,boot_id=1409934065,reason=target_event_count,duration_ms=301717,events=100
```

Data quality:

- BLE captured events: 98/100
- Missing sequences: `39, 72`
...
- P99 latency: `303.3 ms`
- Max latency: `354.5 ms`
- Loss rate: `2.0%`

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R677/raw --run-id W3_ble_auto_heavy --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R677/raw --run-id R677 --run-sheet docs/run_sheet.csv
```
