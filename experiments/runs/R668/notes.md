## R668 - W3 BLE AP-Channel Extension
...
```bash
EXP_RUN_ID=R668 ./run_all.sh --run-id W3_ble_auto_baseline --enable-ble --disable-ack --duration-sec 360 --serial-port /dev/serial-port --serial-baud 115200 --iperf-load 0 --reset-esp32
```

Completion:

```text
EXPERIMENT_COMPLETE,exp=W3_ble_auto,boot_id=1495482824,reason=target_event_count,duration_ms=301994,events=100
```
...
- MAD latency: `83.8 ms`
- P95 latency: `147.9 ms`
- P99 latency: `276.4 ms`
- Max latency: `282.5 ms`
- Loss rate: `1.0%`

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R668/raw --run-id W3_ble_auto_baseline --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R668/raw --run-id R668 --run-sheet docs/run_sheet.csv
```
