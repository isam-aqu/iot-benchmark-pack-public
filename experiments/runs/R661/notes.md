## R661 - W3 BLE AP-Channel Extension
...
```bash
EXP_RUN_ID=R661 ./run_all.sh --run-id W3_ble_auto_heavy --enable-ble --disable-ack --duration-sec 360 --serial-port /dev/serial-port --serial-baud 115200 --iperf-load heavy --reset-esp32
```

Completion:

```text
EXPERIMENT_COMPLETE,exp=W3_ble_auto,boot_id=1323084204,reason=target_event_count,duration_ms=301487,events=100
```
...
- P99 latency: `313.2 ms`
- Max latency: `329.1 ms`
- Loss rate: `1.0%`

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R661/raw --run-id W3_ble_auto_heavy --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R661/raw --run-id R661 --run-sheet docs/run_sheet.csv
```
