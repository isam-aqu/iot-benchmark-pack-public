## R667 - W3 BLE AP-Channel Extension
...
```bash
EXP_RUN_ID=R667 ./run_all.sh --run-id W3_ble_auto_heavy --enable-ble --disable-ack --duration-sec 360 --serial-port /dev/serial-port --serial-baud 115200 --iperf-load heavy --reset-esp32
```

Completion:

```text
EXPERIMENT_COMPLETE,exp=W3_ble_auto,boot_id=998774113,reason=target_event_count,duration_ms=300963,events=100
```

Data quality:

- BLE captured events: 98/100
- Missing sequences: `17, 77`
...
- MAD latency: `84.8 ms`
- P95 latency: `177.3 ms`
- P99 latency: `313.9 ms`
- Max latency: `357.4 ms`
- Loss rate: `2.0%`

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R667/raw --run-id W3_ble_auto_heavy --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R667/raw --run-id R667 --run-sheet docs/run_sheet.csv
```
