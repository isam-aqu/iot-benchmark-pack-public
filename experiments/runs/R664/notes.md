## R664 - W3 BLE AP-Channel Extension
...
```bash
EXP_RUN_ID=R664 ./run_all.sh --run-id W3_ble_auto_baseline --enable-ble --disable-ack --duration-sec 360 --serial-port /dev/serial-port --serial-baud 115200 --iperf-load 0 --reset-esp32
```

Completion:

```text
EXPERIMENT_COMPLETE,exp=W3_ble_auto,boot_id=737490717,reason=target_event_count,duration_ms=298868,events=100
```

Data quality:

- BLE captured events: 98/100
- Missing sequences: `28, 52`
...
- MAD latency: `82.9 ms`
- P95 latency: `179.9 ms`
- P99 latency: `316.1 ms`
- Max latency: `325.1 ms`
- Loss rate: `2.0%`

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R664/raw --run-id W3_ble_auto_baseline --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R664/raw --run-id R664 --run-sheet docs/run_sheet.csv
```
