## R670 - W3 BLE AP-Channel Extension
...
```bash
EXP_RUN_ID=R670 ./run_all.sh --run-id W3_ble_auto_baseline --enable-ble --disable-ack --duration-sec 360 --serial-port /dev/serial-port --serial-baud 115200 --iperf-load 0 --reset-esp32
```

Completion:

```text
EXPERIMENT_COMPLETE,exp=W3_ble_auto,boot_id=1118721861,reason=target_event_count,duration_ms=301276,events=100
```

Data quality:

- BLE captured events: 97/100
- Missing sequences: `38, 83, 84`
...
- MAD latency: `83.4 ms`
- P95 latency: `161.3 ms`
- P99 latency: `175.0 ms`
...
```bash
python3 python/summarize_w3_run.py experiments/runs/R670/raw --run-id W3_ble_auto_baseline --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R670/raw --run-id R670 --run-sheet docs/run_sheet.csv
```
