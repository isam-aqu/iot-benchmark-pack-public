## R665 - W3 BLE AP-Channel Extension
...
```bash
EXP_RUN_ID=R665 ./run_all.sh --run-id W3_ble_auto_heavy --enable-ble --disable-ack --duration-sec 360 --serial-port /dev/serial-port --serial-baud 115200 --iperf-load heavy --reset-esp32
```

Completion:

```text
EXPERIMENT_COMPLETE,exp=W3_ble_auto,boot_id=328109281,reason=target_event_count,duration_ms=300838,events=100
```

Data quality:

- BLE captured events: 99/100
- Missing sequences: `71`
...
- MAD latency: `85.9 ms`
- P95 latency: `173.5 ms`
- P99 latency: `319.3 ms`
- Max latency: `320.2 ms`
- Loss rate: `1.0%`

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R665/raw --run-id W3_ble_auto_heavy --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R665/raw --run-id R665 --run-sheet docs/run_sheet.csv
```
