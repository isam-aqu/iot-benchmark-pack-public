## R695 - W3 Zigbee AP-Channel Extension

Run ID: `W3_zigbee_auto_heavy_ch9`

Protocol: `zigbee`

Purpose: AP channel 9 / 20 MHz overlap heavy-load run paired with baseline `R694`.
...
- Max latency: `13.1 ms`
- Loss rate: `0.0%`
- P95 change vs `R694`: `2.2 ms` (`25.6%`)

Analysis:

```bash
python3 python/summarize_w3_run.py experiments/runs/R695/raw --run-id W3_zigbee_auto_heavy_ch9 --expected-events 100
python3 python/analyze_w3_run.py experiments/runs/R695/raw --run-id R695 --run-sheet docs/run_sheet.csv
```

Interpretation:

This is a valid AP ch9/20 MHz heavy-load replicate paired with `R694`. The reliability caveat is reflected in the data-quality bullets above.
